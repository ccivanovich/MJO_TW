...
CME_num = sum(RMM_thresh_1std(:,j,2,coastal_3));

% Select PG data and calculate risk
CME_risk_alltemps = sum(RMM_thresh_1std(:,:,2,coastal_3),3)./sum(RMM_obs_len(:,2,coastal_3),2)*100;
scrambled_risk_CME = squeeze(sum(RMM_scrambled_thresh(:,:,2,coastal_3scram,:),3))./sum(RMM_MJOdays_scrambled_thresh(:,2,coastal_3scram,:),2)*100;
...
% Calculate station exceedances
NW_num = sum(RMM_thresh_1std(:,j,2,NW_3));

% Select SA data and calculate risk
NW_risk_alltemps = sum(RMM_thresh_1std(:,:,2,NW_3),3)./sum(RMM_obs_len(:,2,NW_3),2)*100;
scrambled_risk_NW = squeeze(sum(RMM_scrambled_thresh(:,:,2,NW_3scram+1,:),3))./sum(RMM_MJOdays_scrambled_thresh(:,2,NW_3scram+1,:),2)*100; % add 1 because removing one station from previous CME list
...
% Calculate station exceedances
SE_num = sum(RMM_thresh_1std(:,j,1,SE_3));

% Select SA data and calculate risk
SE_risk_alltemps = sum(RMM_thresh_1std(:,:,1,SE_3),3)./sum(RMM_obs_len(:,1,SE_3),2)*100;
scrambled_risk_SE = squeeze(sum(RMM_scrambled_thresh(:,:,1,SE_3scram+1,:),3))./sum(RMM_MJOdays_scrambled_thresh(:,1,SE_3scram+1,:),2)*100;
